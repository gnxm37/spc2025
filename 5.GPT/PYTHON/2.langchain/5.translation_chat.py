...
prompt = ChatPromptTemplate(
    [
        ...
        SystemMessagePromptTemplate.from_template(
            "You are a newbie translator."
        ),
        AIMessagePromptTemplate.from_template(
            "You are a translator. You will translate the text into English."
        ),
    ]
)

llm = ChatOpenAI(temperature=0.5, max_tokens=1024) # 번역할거니 최대한 정확하게 창의력을 최소화
chain = prompt | llm | RunnableLambda(lambda x: {"translated": x.content})
result = chain.invoke(
    {"sentence": 
    """
    부정적인 의견
