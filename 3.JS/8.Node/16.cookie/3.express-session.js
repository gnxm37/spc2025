...
// 세션 설정 - 세션 데이터는 기본적으로 서버의 메모리에 암호화 되어서 저장
app.use(session({
...
function visitCounter(req, res, next) {
    ...
    req.session.visitCounter = req.session.visitCounter || 0;
...
app.get('/', (req, res) => {
    ...
    res.send(`잘왔다, 당신의 방문 횟수는: ${req.session.visitCounter}`)
});
...
app.get('/user', (req, res) => {
    ...
    if(username){
        res.send(`당신의 이름은: ${username}입니다. 사용자 정보....(중략)`);
    }else{
...
app.get('/cart', (req, res) => {
    ...
    if(username){
        res.send(`당신의 장바구니에는: ${cart}가 있습니다.`);
    }else{
        res.send('사용자 정보가 없습니다. 로그인하세요.');
    }
